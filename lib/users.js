...
getLinkedIn = function(user){
  // return twitter name provided by user, or else the one used for twitter login
  if(checkNested(user, 'profile', 'linkedin')){
    return user.profile.linkedin;
  }else if(checkNested(user, 'services', 'linkedin', 'screenName')){ // TODO: double-check this with linkedin login
    return user.services.linkedin.screenName;
  }
  return null;
}
getDribble = function(user){
  // return twitter name provided by user, or else the one used for twitter login
  if(checkNested(user, 'profile', 'dribble')){
    return user.profile.dribble;
  }else if(checkNested(user, 'services', 'dribble', 'screenName')){ // TODO: double-check this with dribble login
    return user.services.dribble.screenName;
  }
  return null;
}
